...
class SaleReturnController extends Controller
{
    ...
    {
        $query = SaleReturn::with(['customer', 'status', 'warehouse', 'paymentMethod', 'details.product', 'createdBy', 'updatedBy']);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->products as $item) {
                ...
                $inventories = Inventory::where('product_id', $saleDetail->product_id)
    ...
    {
        $sale_return = SaleReturn::with(['customer', 'status', 'paymentMethod', 'details.product', 'createdBy', 'updatedBy'])->findOrFail($id);
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            foreach ($request->products as $item) {
                ...
                $negatives = Inventory::where('product_id', $saleDetail->product_id)
                    ->where('warehouse_id', $saleReturn->warehouse_id)
                    ->where('qty', '<', 0)
                    ->orderBy('created_at')
                    ->lockForUpdate()
